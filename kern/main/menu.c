...
struct thread *menu_thread;
struct thread *curthread;

struct lock *menu_thread_lock;
struct cv *menu_thread_cv;
...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	menu_thread = curthread;
	menu_thread->t_proc = process_bootstrap();
...
common_prog(int nargs, char **args)
{
	...
	result = thread_fork(args[0] /* thread name */,
			args /* thread argargs */, nargs /* thread arg */,
			cmd_progthread, NULL);
	if (result) {
		...
		menu_thread_lock = lock_create("menu_thread_lock");
		menu_thread_cv = cv_create("menu_thread_cv");

		lock_acquire(menu_thread_lock);
		cv_wait(menu_thread_cv, menu_thread_lock);
...
cmd_dbflags(int nargs, char **args)
{
	...
	showmenu("Debug flags", debugmenu);
...
static
int
dbflags_args(int nargs, char **args)
{
	//kprintf("%s\n", args[1]);
	//kprintf("%s\n", args[2]);
