 ...
 */
 ...
 load_elf(struct vnode *v, vaddr_t *entrypoint)
 {
 	 ...
 	 */
	 	...
	 	//To enable the load on demand we memoize some vnode data. We store the OFFSET
		//in the for loop

	(curthread->t_vmspace->as_v).as_vnode = v; //keep a reference to the vnode

 	for (i=0; i<eh.e_phnum; i++) {
 		assert(eh.e_phnum == 3); // HACKS
 		// kprintf("%d\n", i);
 		...
 		switch (ph.p_type) {
			...
			(curthread->t_vmspace->as_v).offset[i-1] = ph.p_offset;
			(curthread->t_vmspace->as_v).filesize[i-1] = ph.p_filesz;
 	}
 	...
 	//At this point we have all the segments (text, data, heap and stack) base &
 	//bounds defined. However need to load the segment on demand. We do this upon
 	//a page fault in vm_fault().

 	//Carry on, and do the actual loading in vm_fault.
  ...
  //
 	// 		off_t offset = eh.e_phoff + i*eh.e_phentsize;
	// 		(curthread->t_vmspace->as_v).offset[i] = offset;
 	// 	}

		//Just a quick check to ensure every segment has been set up properly

		int err = as_complete_load(curthread->t_vmspace);
		if (err) {
			return (err);
		}
